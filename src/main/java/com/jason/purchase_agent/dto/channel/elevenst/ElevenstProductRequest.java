...
public class ElevenstProductRequest {
    // ◎해외상품코드, D(현지온라인쇼핑몰)
    ...
    public static ElevenstProductRequest fromForm(ProductRegistrationRequest request) {
        ...
        // 상품 고시 정보 분기
        ElevenstProductRequest.ProductNotification notification = null;
        if ("FOOD".equals(productDto.getProductType())) {
            notification = ElevenstProductRequest.createFoodNotification();
        } else if ("HEALTH".equals(productDto.getProductType())) {
            notification = ElevenstProductRequest.createHealthNotification();
        }

        return ElevenstProductRequest.builder()
                // ● 표시된 필드만 직접 세팅
