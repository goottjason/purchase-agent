...
public interface ProcessStatusRepository extends JpaRepository<ProcessStatus, Long> {
    ...
    /*@Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("SELECT ps FROM ProcessStatus ps WHERE ps.batchId = :batchId AND ps.productCode = :productCode")
    ProcessStatus findForUpdate(@Param("batchId") String batchId, @Param("productCode") String productCode);*/
