...
public interface ProductRepository extends JpaRepository<Product, String> {
    ...
    @Query("SELECT p FROM Product p " +
            ...
            "WHERE " +
            // 상품코드 IN 검색 조건 (우선순위 높음)
            "(:#{#productCodes == null} = true OR p.code IN :productCodes) " +
            // 키워드 검색 조건
            ...
            " LOWER(p.memo) LIKE LOWER(CONCAT('%', :keyword, '%'))) " +
            // 공급업체 필터 조건
    ...
    Page<Product> findProductsWithFilters(
            ...
            @Param("productCodes") List<String> productCodes,  // ⭐ 새로 추가
